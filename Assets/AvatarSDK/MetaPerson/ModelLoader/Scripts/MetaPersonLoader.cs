...
namespace AvatarSDK.MetaPerson.ModelLoader
{
	public class MetaPersonLoader : MonoBehaviour
	{
		...
		public async Task LoadModelAsync(string uri, Action<float> downloadProgressCallback = null)
		{
			...
			if (success)
			{
				...
				await gltfImporter.InstantiateMainSceneAsync(avatarObject.transform);
			}
			else
			{
